...
function parseBreakHours(breakTime) {
    if (!breakTime)
        return 0;
    const n = parseFloat(String(breakTime));
    return Number.isFinite(n) ? n : 0;
}
function parseShiftTimes(shiftTime) {
...
function deriveStatus(startDate, endDate, todayStart) {
    if (endDate)
        return 'completed';
...
class ShiftAssignmentService {
    serialize(row, todayStart = todayUtcBounds().start) {
        ...
        const status = deriveStatus(new Date(row.start_date), row.end_date ? new Date(row.end_date) : null, todayStart);
        return {
            ...row,
    ...
    async assignShift(data) {
        ...
        const existing = await client_1.prisma.shiftAssignment.findFirst({
            where: {
        ...
        if (existing) {
            throw new apiError_1.AppError(400, `A shift already exists for ${employee.name} on ${startDate.toISOString().slice(0, 10)}`);
        }
        try {
            const assignment = await client_1.prisma.shiftAssignment.create({
                data: {
                    employee_id: data.employee_id,
            ...
            if (err?.code === 'P2002') {
                throw new apiError_1.AppError(400, `A shift already exists for ${employee.name} on that date`);
            }
            throw err;
        }
    }
    async getCurrentShift(employee_id) {
        const row = await client_1.prisma.shiftAssignment.findFirst({
    ...
    async getShiftHistory(employee_id) {
        ...
        const rows = await client_1.prisma.shiftAssignment.findMany({
            where: { employee_id },
            orderBy: { start_date: 'desc' },
            include: { employee: { select: employeeSelect } },
        });
    ...
    async endCurrentShift(employee_id, end_date = new Date(), sales) {
        const open = await client_1.prisma.shiftAssignment.findFirst({
            where: { employee_id, end_date: null },
            orderBy: { start_date: 'desc' },
        });
        if (!open)
            throw new apiError_1.AppError(404, 'No open shift found for this employee');
        const updated = await client_1.prisma.shiftAssignment.update({
            where: { id: open.id },
            data: {
                end_date,
    ...
    async endShiftById(id, sales) {
        const existing = await client_1.prisma.shiftAssignment.findUnique({ where: { id } });
        if (!existing)
            throw new apiError_1.AppError(404, 'Shift assignment not found');
        if (existing.end_date) {
            throw new apiError_1.AppError(400, 'This shift is already completed');
        }
        const updated = await client_1.prisma.shiftAssignment.update({
            where: { id },
            data: {
    ...
    async listShifts(params) {
        ...
        const [rows, total, allForSummary] = await Promise.all([
            client_1.prisma.shiftAssignment.findMany({
                where,
                skip,
                take: limit,
            ...
            client_1.prisma.shiftAssignment.count({ where }),
            client_1.prisma.shiftAssignment.findMany({
                select: {
                    start_date: true,
                    end_date: true,
        ...
        for (const row of allForSummary) {
            const status = deriveStatus(row.start_date, row.end_date, todayStart);
            if (status === 'active')
                summary.active += 1;
            ...
            if (row.start_date >= todayStart && row.start_date <= todayEnd) {
                ...
                const { startTime, endTime } = parseShiftTimes(row.shift_time);
                summary.todayHours += calculateHours(startTime, endTime, parseBreakHours(row.break_time));
            }
        }
    ...
    async updateShift(id, data) {
        const existing = await client_1.prisma.shiftAssignment.findUnique({
            where: { id },
        ...
        if (data.start_date !== undefined) {
            const startDate = normalizeStartDate(data.start_date);
            const clash = await client_1.prisma.shiftAssignment.findFirst({
                where: {
            ...
            if (clash) {
                throw new apiError_1.AppError(400, `A shift already exists for ${existing.employee?.name || 'this employee'} on ${startDate.toISOString().slice(0, 10)}`);
            }
        ...
        try {
            ...
            if (err?.code === 'P2002') {
                throw new apiError_1.AppError(400, 'A shift already exists for that employee on that date');
            }
            throw err;
        }
    }
    async deleteShift(id) {
        ...
        if (!existing)
            throw new apiError_1.AppError(404, 'Shift assignment not found');
        await client_1.prisma.shiftAssignment.delete({ where: { id } });
