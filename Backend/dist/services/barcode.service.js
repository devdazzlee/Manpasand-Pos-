...
class BarcodeService {
    ...
    async getAvailablePrinters() {
        try {
            ...
            try {
                ...
                logger_1.logger.warn('PowerShell method failed, trying alternative:', psError);
                ...
                try {
                    ...
                    logger_1.logger.warn('WMIC method failed, trying registry method:', wmicError);
                    ...
                    try {
                        ...
                        logger_1.logger.error('All printer detection methods failed:', regError);
            ...
            logger_1.logger.info(`Found ${printers.length} printers`);
            return printers;
        }
        catch (error) {
            logger_1.logger.error('Error getting printers:', error);
    ...
    async testPrinterConnection(printerName) {
        try {
            ...
            try {
                ...
                logger_1.logger.warn('PowerShell printer test failed, trying alternative:', psError);
                ...
                try {
                    ...
                    logger_1.logger.warn('WMIC printer test failed:', wmicError);
                }
            }
            ...
            logger_1.logger.error('Error testing printer:', error);
            return {
                success: false,
    ...
    generateEPL(product, settings = {}) {
        const name = (product.name || "").toUpperCase();
        const weight = this.formatWeightDisplay(product.netWeight);
        const price = Math.round(Number(this.calculatePriceByWeight(product.netWeight, product.sales_rate_exc_dis_and_tax)));
...
P1`;
    ...
    async printBarcodes(products, printerName, settings = {}) {
        try {
            ...
            try {
                ...
                logger_1.logger.info('Cleared existing print jobs');
            }
            catch (clearError) {
                logger_1.logger.warn('Could not clear print jobs:', clearError);
            }
            ...
            try {
                ...
                logger_1.logger.info('Printer info:', printerInfo);
            }
            catch (infoError) {
                logger_1.logger.warn('Could not get printer info:', infoError);
            }
            ...
            try {
                ...
                logger_1.logger.info('EPL file also saved to desktop:', desktopPath);
            }
            catch (desktopError) {
                logger_1.logger.warn('Could not save to desktop:', desktopError);
            }
...
echo Print job completed`;
            ...
            logger_1.logger.info('Batch file created:', batchFile);
            // Log the EPL content for debugging
            logger_1.logger.info('EPL Content:', allEplCommands);
            logger_1.logger.info('Temp file created:', tempFile);
            try {
                ...
                try {
                    logger_1.logger.info(`Attempting multiple printing methods to: ${printerName}`);
                    ...
                    for (const method of methods) {
                        try {
                            logger_1.logger.info(`Trying method: ${method}`);
                            await execAsync(method);
                            printSuccess = true;
                            logger_1.logger.info(`Method successful: ${method}`);
                            break;
                        }
                        catch (methodError) {
                            logger_1.logger.warn(`Method failed: ${method}`, methodError);
                        }
                    }
                    if (printSuccess) {
                        logger_1.logger.info('One of the printing methods was successful');
                    }
                    else {
                    ...
                    logger_1.logger.warn('Windows print command failed, trying alternative approach:', printError);
                    printError = printError;
                    // Method 2: Try PowerShell with binary data
                    try {
                        logger_1.logger.info(`Attempting PowerShell binary printing`);
                        ...
                        printSuccess = true;
                        logger_1.logger.info('PowerShell binary printing successful');
                    }
                    catch (psError) {
                        logger_1.logger.warn('PowerShell binary printing failed, trying .prn file method:', psError);
                        printError = psError;
                        // Method 3: Try direct port using printer's actual port
                        try {
                            logger_1.logger.info(`Attempting direct port printing using printer port`);
                            ...
                            logger_1.logger.info(`Printer port: ${printerPort}`);
                            // Send ZPL directly to the printer's port
                            await execAsync(`type "${tempFile}" > "${printerPort}"`);
                            printSuccess = true;
                            logger_1.logger.info('Direct port printing successful');
                        }
                        catch (portError) {
                            logger_1.logger.warn('Direct port failed, trying batch file:', portError);
                            printError = portError;
                            // Method 4: Try batch file approach
                            try {
                                await execAsync(`"${batchFile}"`);
                                printSuccess = true;
                                logger_1.logger.info('Batch file printing successful');
                            }
                            catch (batchError) {
                                logger_1.logger.error('All printing methods failed:', batchError);
                ...
                if (printSuccess) {
                    logger_1.logger.info(`Successfully printed ${products.length} barcodes to ${printerName}`);
            ...
            logger_1.logger.error('Error printing barcodes:', error);
    ...
    parseWeightToGrams(weightInput) {
