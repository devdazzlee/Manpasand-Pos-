...
import { useSizes, useSizeMutations } from "@/hooks/queries/use-sizes";
import type { Size as SizeRow } from "@/lib/api/sizes";
...
const Sizes: React.FC = () => {
  ...
  const { sizes, isFirstLoad, isRefreshing, refetch } = useSizes({
    search: debouncedSearch || undefined,
  });
  const { create, update, remove } = useSizeMutations();

  const [addOpen, setAddOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const [detailTarget, setDetailTarget] = useState<SizeRow | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<SizeRow | null>(null);
  const [current, setCurrent] = useState<SizeRow | null>(null);
  ...
  const submit = () => {
    ...
    const onError = (e: unknown) => {
      const msg = extractApiError(e, "Failed to save size");
      setFormError(msg);
      toast({ variant: "destructive", title: "Could not save size", description: msg });
    };
    ...
    remove.mutate(target.id, {
      ...
      onError: (e) =>
        toast({
          ...
          description: extractApiError(e, "Failed to delete size"),
        }),
    });
  };

  const stats = useMemo(() => {
    const active = sizes.filter((s) => s.is_active).length;
    return {
      total: sizes.length,
      active,
      inactive: sizes.length - active,
      products: sizes.reduce((sum, s) => sum + (s.product_count || 0), 0),
    };
  }, [sizes]);
  ...
  }, [sizes, statusFilter, sortOrder]);

  return (
    <>
      <PageHeader
        title="Sizes"
        description={`${stats.total} size${stats.total === 1 ? "" : "s"}`}
        actions={
          <>
            <Button
              variant="outline"
      ...
      <PageBody className="space-y-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {(
            [
              { label: "Total sizes", value: stats.total, icon: Ruler },
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between py-3">
            <CardTitle className="text-sm">
              Sizes <span className="text-muted-foreground">({filtered.length})</span>
            </CardTitle>
            {isRefreshing && (
            ...
            <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Enable or disable this size in system workflows
                </p>
              </div>
              <Switch
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              moved to the default size and are not deleted. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
